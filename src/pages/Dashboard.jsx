...
export default function Dashboard() {
  ...
  const fixAccount = async () => {
    ...
    try {
      ...
      const { error: pError } = await supabase
        .from('user_profiles')
        .upsert({ id: user.id, name: user.email.split('@')[0], "tenantId": newTenant.id, role: 'admin' });

      if (pError) throw pError;

      await supabase.from('user_tenants').insert({ user_id: user.id, tenant_id: newTenant.id, role: 'admin' });
  ...
  const copyJobLink = (e, job) => {
    e.stopPropagation(); // Bloqueio extra no clique
    ...
    <div className="p-8 max-w-7xl mx-auto">
      ...
      <div className="flex flex-col md:flex-row justify-between items-center mb-8 gap-4 border-b pb-4">
        ...
        <div className="flex items-center gap-3">
            {/* BOTÃO APROVADOS (NOVO) */}
            <button 
                onClick={() => navigate('/hiring')} 
                className="bg-white text-gray-700 border border-gray-300 px-4 py-2 rounded hover:bg-gray-50 flex items-center shadow-sm transition font-medium"
            >
                <CheckCircle className="w-4 h-4 mr-2 text-green-600" /> Aprovados
            </button>

            {profile?.tenantId && (
                <button 
                    onClick={() => setIsModalOpen(true)}
                    className="bg-blue-600 text-white px-4 py-2 rounded hover:bg-blue-700 flex items-center shadow transition font-medium"
                >

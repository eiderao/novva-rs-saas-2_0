...
export default function ApplicationDetails() {
  ...
  const fetchData = async () => {
    ...
    try {
      ...
      // 1. Busca dados da aplicação e candidato
      ...
      // Parse Seguro do formData (pode ser string ou jsonb)
      ...
      // 2. Busca dados da Vaga (para pegar os parâmetros de avaliação)
      let jobParams = {};
      ...
      // 3. Busca TODAS as avaliações para esta aplicação (Cálculo 360º)
      const { data: allEvals } = await supabase.from('evaluations').select('*').eq('application_id', appId);
      
      if (allEvals) {
          // Mapeia IDs de avaliadores para Nomes (para o histórico)
          ...
          // Separa a avaliação do usuário atual (se houver) para pré-preencher o form
          if (user) {
              const myEval = allEvals.find(e => e.evaluator_id === user.id);
              if (myEval) {
                  // Aqui garantimos que o EvaluationForm receba os dados mastigados
                  // Se myEval.scores existir, usa ele. Se não, tenta a estrutura raiz.
                  const sourceScores = myEval.scores || myEval;
                  
                  setCurrentUserEvaluation({ 
                      ...sourceScores, 
                      anotacoes_gerais: myEval.notes || sourceScores.anotacoes_gerais, 
                      // O score final calculado aqui é apenas para exibição no badge, o form recalcula em tempo real
